...
export default function UsersPageCreateAction() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object().shape({
      fullName: Yup.string().min(2, 'Слишком короткое!').max(50, 'Слишком длинное!').required('Обязательное поле'),
      phone: Yup.string()
      ...
      >
        <form onSubmit={formik.handleSubmit} className={'flex flex-wrap gap-6'}>
          ...
          <label className={'account-form__input flex-1'} htmlFor='cleanm'>
            E-mail
            <input id='cleanm' name='cleanm' type='email' onChange={formik.handleChange} value={formik.values.cleanm} />
          </label>

          <Select
            variant={'static'}
            label={'Тип'}
            className={'bg-white account-form__input  flex-1'}
            labelProps={{ className: 'text-[var(--active] relative block' }}
            name={'user_type'}
          >
          ...
          <Select
            variant={'static'}
            className={'bg-white account-form__input flex-1 flex flex-grow-1 w-full'}
            labelProps={{ className: 'text-[var(--active] relative block' }}
            placeholder={'Выбрать статус'}
            label={'Статус'}
            name={'user_is_active'}
            defaultValue={'Выбрать статус'}
          >
            <Option className={'relative'} value='active'>
              Активный
            </Option>
