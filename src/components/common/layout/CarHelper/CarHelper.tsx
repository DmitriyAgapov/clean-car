...
const CarHelper = ():JSX.Element => {
	...
	const Header = () => <ul className={'subgrid col-span-full bg-[#090909] !mt-0 pt-3 pb-2.5 !mb-8 tablet-max:hidden'} >{headerValues.map((item, index) => <li className={`${index === 0 && 'col-start-2'} text-[#929398] text-xss uppercase`} key={`${index}_listEl`}>{item}</li> )}</ul>
