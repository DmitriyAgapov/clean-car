...
const TabHistory = ({company_id, user_id, company_type, state }:any) => {
	const store = useStore()
	const params = useParams()
	const localStoreF = useLocalStore<LocalRootStore>(() => localRootStoreF)
	const {isLoading, data} = useSWR([`users_history_${user_id}`, company_type, user_id,localStoreF.params.getSearchParams] , ([url, company_type, companyId, args]) => agent.Users.getHistoryUser(company_id, user_id).then(r => r.data))
...
export default observer(TabHistory)
