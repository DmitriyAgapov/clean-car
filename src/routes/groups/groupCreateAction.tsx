...
export default function GroupPageCreateAction(props: any) {
  ...
  return (
    <Section type={SectionType.default}>
      ...
      ></Panel>
      <Panel
        className={'col-span-full py-7 px-14 grid grid-rows-[1fr_auto]'}
        footer={
      ...
      >
        <div
          className={
            'accounts-group_body text-[#606163] gap-x-4 gap-y-16 grid grid-cols-[1.25fr_2fr] grid- font-medium'
          }
        >
          <div>
            <Heading text={'Введите название группы'} color={HeadingColor.accent} variant={HeadingVariant.h5} />
          </div>
          <div>
            <input type={'text'} name={'name'} className={'h-10'} value={changes.name} onChange={handleChangeName} />
          </div>
          <div>
            <Heading text={'Присвойте права группе'} color={HeadingColor.accent} variant={HeadingVariant.h5} />
          </div>
          <div>
